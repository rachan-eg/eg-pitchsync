    ...
    def generate(self, force: bool = False) -> Path:
        ...
        for i, (phase_name, phase_data) in enumerate(phases_sorted):
            ...
            scoring_data = [
                ...
                [Paragraph("Time Penalty", self.styles["TableCell"]), Paragraph(f'<font color="{DANGER.hexval()}">{int(metrics.time_penalty)}</font>' if metrics.time_penalty > 0 else f"{int(metrics.time_penalty)}", self.styles["TableCellCenter"]), Paragraph(f'<font color="{DANGER.hexval()}">{"−" if w_time > 0 else ""}{w_time}</font>', self.styles["TableCellCenter"])],
                [Paragraph("Retry Count", self.styles["TableCell"]), Paragraph(f'<font color="{DANGER.hexval()}">{int(metrics.retries)}</font>' if metrics.retries > 0 else f"{int(metrics.retries)}", self.styles["TableCellCenter"]), Paragraph(f'<font color="{DANGER.hexval()}">{"−" if w_retry > 0 else ""}{w_retry}</font>', self.styles["TableCellCenter"])],
                [Paragraph("Hint Penalty", self.styles["TableCell"]), Paragraph(f'<font color="{DANGER.hexval()}">{int(metrics.hint_penalty)}</font>' if metrics.hint_penalty > 0 else f"{int(metrics.hint_penalty)}", self.styles["TableCellCenter"]), Paragraph(f'<font color="{DANGER.hexval()}">{"−" if w_hint > 0 else ""}{w_hint}</font>', self.styles["TableCellCenter"])],
                [Paragraph("Efficiency Bonus", self.styles["TableCell"]), Paragraph(f'<font color="{SUCCESS.hexval()}">{"+" if int(metrics.efficiency_bonus) > 0 else ""}{int(metrics.efficiency_bonus)}</font>' if metrics.efficiency_bonus > 0 else f"{int(metrics.efficiency_bonus)}", self.styles["TableCellCenter"]), Paragraph(f'<font color="{SUCCESS.hexval()}">{"+" if w_bonus > 0 else ""}{w_bonus}</font>', self.styles["TableCellCenter"])],
